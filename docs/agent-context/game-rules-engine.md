...
ends. Each end zone uses that roster's palette; Human-derived teams own the
row-0 presentation and Orc-derived teams own row 25. This is presentation only
and does not change touchdown detection.
...
- The live universe view is a parent-child tree: each block owns its resulting
  ...
  **Needs a plan** (and their empty ancestry) without conceding them or changing
  ...
  and its controls fill the same compact fixed-width track regardless of which
  controls are present. Tracks have only a small gutter, and each block header
