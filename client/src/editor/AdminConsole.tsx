...
export function AdminConsole({ idToken, onBack }: Props) {
  ...
  return <section className="admin-console">
    ...
    <div className="admin-console__notice">Managed administrators take effect immediately. Deployment administrators configured through <code>ADMIN_EMAILS</code> remain outside this list.</div>
    {error && <div className="admin-console__error" role="alert">{error}</div>}
    <section className="admin-console__panel"><h2>Managed administrators</h2><p>Add verified Google email addresses. At least one managed administrator must remain when no deployment administrator is configured.</p>
      ...
      {access ? <><p className="admin-console__count">{access.managedAdmins.length} managed · {access.configuredAdminCount} deployment-configured</p><ul>{access.managedAdmins.map(address => <li key={address}><span>{address}</span><button className="btn btn--danger" disabled={busy} onClick={() => setPendingRemoval(address)}>Remove</button></li>)}</ul>{access.managedAdmins.length === 0 && <p>No managed administrators yet.</p>}</> : <p>Loading access controls…</p>}
    </section>
